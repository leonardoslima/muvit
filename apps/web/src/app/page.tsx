    ...
    <div className="bg-mkt-light text-foreground">
          ...
          <div className="grid items-center gap-12 lg:grid-cols-[1.15fr_1fr]">
            <div className="flex flex-col gap-8">
              ...
              <p className="max-w-2xl text-lg leading-relaxed text-mkt-on-dark/85 lg:text-hero-lead">
                Cadastre alunos, monte planos a partir da maior biblioteca de exercícios e
                acompanhe a evolução com avaliações físicas e logs em tempo real — num só lugar.
              </p>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Button asChild variant="outline" size="xl" className="border-white/30 bg-white/5 text-mkt-on-dark hover:bg-white/15">
      ...
      <Section bg="dark" id="problem">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {[
            { title: 'Planilhas que somem', body: 'Treinos espalhados em PDFs, prints e papéis que ninguém acha quando precisa.' },
            { title: 'Sem visibilidade do aluno', body: 'Você não sabe se ele treinou, com que carga, ou quanto evoluiu desde a última avaliação.' },
            { title: 'Mensagens infinitas', body: 'WhatsApp como CRM, prontuário e biblioteca ao mesmo tempo. Não escala.' },
          ].map((c) => (
            <article key={c.title} className="rounded-lg border border-white/10 bg-mkt-dark-elevated p-6">
      ...
      <Section bg="light" id="features">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {[
            { icon: Users, title: 'Gestão de alunos', body: 'Perfis completos com objetivos, restrições, histórico e status. Convide via link.' },
            { icon: ClipboardList, title: 'Editor de treinos', body: 'Monte planos com até 7 dias, séries, repetições, carga, descanso e tempo.' },
            { icon: Dumbbell, title: 'Biblioteca de exercícios', body: 'Globais cobrindo principais grupos musculares + customs ilimitados.' },
            { icon: LineChart, title: 'Avaliações + evolução', body: 'Peso, % de gordura e medidas com gráfico de evolução automático.' },
            { icon: Smartphone, title: 'App do aluno', body: 'Aluno vê o treino do dia, registra séries e cargas direto no celular.' },
            { icon: MessageSquareHeart, title: 'Notificações', body: 'Lembretes diários e atualizações importantes via push e e-mail.' },
          ].map((f) => (
      ...
      <Section bg="light" id="testimonials">
        <SectionHead eyebrow="Quem usa" title="Personal trainers brasileiros estão escalando com a Muvit." />
        <div className="grid gap-5 lg:grid-cols-3">
          {[
            { name: 'Renata Souza', role: 'Personal · São Paulo', quote: '"Saí de planilhas e WhatsApp. Hoje recebo logs de treino dos meus 32 alunos sem precisar perguntar."' },
            { name: 'Carlos Andrade', role: 'Personal · Rio de Janeiro', quote: '"Os gráficos de evolução fecham a venda na 1ª reavaliação. Aluno vê o progresso e renova."' },
            { name: 'Aline Pires', role: 'Aluna independente', quote: '"Comecei a treinar sem personal e o Muvit me deu estrutura. Em 3 meses já tinha histórico para mostrar."' },
          ].map((t) => (
            <article key={t.name} className="flex flex-col gap-5 rounded-lg border border-mkt-card-border bg-card p-6 shadow-card">
      ...
      <Section bg="dark" id="how">
        <SectionHead eyebrow="Como funciona" title="Do cadastro à primeira avaliação em 5 minutos." dark />
        <ol className="grid gap-4 lg:grid-cols-4">
          {[
            ...
            <li key={s.n} className="rounded-lg border border-white/10 bg-mkt-dark-elevated p-6 text-mkt-on-dark">
      ...
      <Section bg="light" id="pricing">
        <SectionHead eyebrow="Planos" title="Comece grátis. Faça upgrade quando seu negócio pedir." />
        <div className="grid gap-6 lg:grid-cols-3">
          <PriceCard name="Solo" price="Grátis" tagline="Para começar" features={['Até 5 alunos', 'Biblioteca global', 'Avaliações físicas', 'App do aluno']} />
          <PriceCard name="Pro" price="R$ 79" unit="/mês" tagline="Mais popular" highlight features={['Alunos ilimitados', 'Exercícios customs ilimitados', 'Notificações push + e-mail', 'Relatórios de evolução', 'Suporte prioritário']} />
          <PriceCard name="Studio" price="R$ 199" unit="/mês" tagline="Para academias" features={['Tudo do Pro', 'Multi-personal (até 5)', 'Marca personalizada', 'Acesso à API']} />
        </div>
      </Section>
      ...
      <Section bg="light" id="faq">
        ...
        <div className="mx-auto max-w-3xl divide-y divide-mkt-card-border rounded-lg border border-mkt-card-border bg-card">
          {[
            { q: 'Posso testar grátis?', a: 'Sim. O plano Solo é grátis para sempre, com até 5 alunos. Sem cartão.' },
            { q: 'Meu aluno também precisa pagar?', a: 'Não. O acesso do aluno via app é incluído em qualquer plano.' },
            { q: 'Funciona offline?', a: 'O app do aluno tem cache local — ele consegue ver o treino e registrar séries mesmo sem internet.' },
            { q: 'Como migro de planilhas?', a: 'Importe seus alunos via CSV. Treinos novos você monta direto no editor.' },
          ].map((item) => (
      ...
      <section className="bg-mkt-hero py-24 text-center text-mkt-on-dark">
        <div className="mx-auto flex max-w-3xl flex-col items-center gap-6 px-6">
          ...
          <p className="text-lg text-mkt-on-dark/85">Começa em menos de 2 minutos. Sem cartão de crédito.</p>
          <Button asChild variant="cta" size="xl">
            <Link href="/signup" className="gap-2">
      ...
      <div className="mx-auto flex max-w-container items-center justify-between px-6 py-5 lg:px-20">
        ...
        <div className="hidden items-center gap-8 lg:flex">
          {[
            ...
            <Link key={href} href={href!} className="text-sm font-medium text-mkt-on-dark/85 hover:text-mkt-on-dark">
              {label}
            </Link>
          ))}
        </div>
        <div className="flex items-center gap-2">
          <Button asChild variant="outline" size="sm" className="border-white/25 bg-white/5 text-mkt-on-dark hover:bg-white/15">
      ...
      <div className="rounded-lg border border-white/15 bg-card p-6 shadow-elevated">
        ...
        <ul className="mt-4 flex flex-col gap-3 text-sm text-foreground">
          {[
            ...
            <li key={name} className="flex items-center justify-between border-b border-border pb-3 last:border-b-0">
...
function Section({
  ...
  return (
    <section id={id} className={`${bg === 'dark' ? 'bg-mkt-dark text-mkt-on-dark' : 'bg-mkt-light'} py-24 lg:py-32`}>
...
function SectionHead({
  ...
  return (
    <header className="flex max-w-3xl flex-col gap-4">
      <span className="font-display text-[11px] font-semibold uppercase tracking-[0.12em] text-primary">{eyebrow}</span>
      <h2 className={`font-display text-3xl font-bold leading-tight tracking-tight lg:text-hero-h2 ${dark ? 'text-mkt-on-dark' : 'text-foreground'}`}>
        {title}
      </h2>
      {subtitle && (
        <p className={`text-base leading-relaxed ${dark ? 'text-mkt-on-dark-muted' : 'text-muted-foreground'}`}>
          {subtitle}
        </p>
      )}
    </header>
...
function PriceCard({
  ...
  return (
    <article className={`flex flex-col gap-6 rounded-lg p-8 ${highlight ? 'border-2 border-primary bg-card shadow-elevated' : 'border border-mkt-card-border bg-card shadow-card'}`}>
      <div className="flex items-center justify-between">
      ...
      <div className="mx-auto flex max-w-container flex-col gap-12 px-6 lg:px-20">
        <div className="grid gap-10 lg:grid-cols-[1.5fr_1fr_1fr_1fr]">
          <div className="flex flex-col gap-3">
            <Logo variant="on-dark" size="md" />
            <p className="max-w-sm text-sm">A plataforma de treinos para personal trainers e alunos independentes.</p>
          </div>
          <FooterCol title="Produto" links={[['Funcionalidades', '#features'], ['Planos', '#pricing'], ['FAQ', '#faq']]} />
          <FooterCol title="Empresa" links={[['Sobre', '#'], ['Blog', '#'], ['Contato', 'mailto:hi@muvit.app']]} />
